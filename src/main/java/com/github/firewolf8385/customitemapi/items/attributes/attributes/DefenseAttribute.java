...
public class DefenseAttribute extends ItemAttribute {
        ...
        switch (customItem.getType()) {
            case HELMET, CHESTPLATE, LEGGINGS, BOOTS -> {
                Arrays.asList(EquipmentSlot.HAND, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET).forEach(slot -> {
                    AttributeModifier modifier = new AttributeModifier(UUID.randomUUID(), "generic.armor", value, AttributeModifier.Operation.ADD_NUMBER, slot);
                    item.addAttributeModifier(attribute, modifier);
                });
            }

            default -> {
                AttributeModifier modifier = new AttributeModifier(UUID.randomUUID(), "generic.armor", value, AttributeModifier.Operation.ADD_NUMBER, EquipmentSlot.HAND);
                item.addAttributeModifier(attribute, modifier);
            }
        }
    }
